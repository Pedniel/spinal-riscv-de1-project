...
class FreqDiv() extends Component {
  ...
  // Registers
  val frac_acc = Reg(UInt(4 bits)) init(0)
  val ctr      = Reg(UInt(8 bits)) init(0)
  
  // comb logic
  val overflow = UInt(1 bits)
  
  io.ctr_o := ctr
  overflow := ((frac_acc +^ io.div_in(3 downto 0))(4)).asUInt  // Detect carry
  // FSM
  when (overflow === U(1)) {
    frac_acc := io.div_in(3 downto 0) // Reset to frac_acc + div_in, not just frac_acc
    io.en_o := False
